...
async function main() {
    ...
    await ust.approve(swapRouter.address, ethers.constants.MaxUint256).then(tx => tx.wait());

    await swapRouter.addLiquidity(
        dai.address, 
        ust.address, 
        hre.ethers.utils.parseEther(daiUST[0]), 
        hre.ethers.utils.parseEther(daiUST[1]), 
        "0", 
        "0", 
        owner.address, 
