...
async function main() {
    ...
    const pairAddr = await swapFactory.getPair(dai.address, wkub.address);
    const kubdaiPair = await BlockSwapPair.attach(pairAddr);

    const lp = await kubdaiPair.balanceOf(owner.address);

    await kubdaiPair.approve(swapRouter.address, hre.ethers.constants.MaxUint256);
